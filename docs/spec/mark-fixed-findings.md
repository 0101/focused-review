...
- Add `fixed: new Set()` to the `state` object (~507) and, in `restoreState()` (template ~680-708, fixture ~540): **rebuild** `state.fixed` (and `state.disregarded`) **fresh** each call — `state.disregarded = new Set(); state.fixed = new Set();` *before* reseeding from the server-rendered `.dimmed`/`.fixed` classes, then prune to present ids and re-apply `classList.toggle("fixed", …)`, mirroring `dimmed`; reconcile-from-server-only (no optimistic add). The reset is load-bearing, **not** an add-only union (F2): the single canvas file is morphed in place and its JS state survives across runs (`window.__frCanvasInit`), and `record_id`s are positional (`f1..fN` every run), so a stale mark unioned from a prior run would be re-applied to a still-present id even after a new `run_id` correctly dropped the class. (`state.selected` is genuine client state and is intentionally **not** reset — only the server-derived sets are.) Leave the rule-fix preview block untouched.
...
- **Finding ids are `f#` (rendered `F#`), never `r#`/`(rN)`.** The renderer/validator contract is `record_id = f"f{position}"` with `_FINDING_ID_RE = ^f[0-9]+$` (`focused-review.py`), and review.md headings are `### F#.` — globally unique, with no per-section display-number restart and no `(rN)` anchor. `validate-action --apply-fixed` rejects an `r#` id (exit 1, *before* the persist runs), so any mark-fixed instruction that constructs `r#` ids silently never writes the `fixed` state. The Step 6d Findings sub-path, the Step 6e terminal-fix path, and the Post-Mortem heading description must therefore use `f#`/`### F#.`. (Corrected in `tm-mark-fixed-findings-rv1`; the remaining pre-existing `r#` labels in the *descriptive* canvas-action intro prose — `SKILL.md` 345/353/355 — and in `focused-review.py` error strings / argparse help / docstrings were finished in `tm-mark-fixed-findings-yt3`. The one intentional `(rN)` remnant is `_md_finding_block`'s comment that the *old* `### {n}. (rN)` heading shape (D-23) is gone — a factual record of the rejected scheme, not a live instruction.)
- **`restoreState()` rebuilds the server-derived sets fresh, it does not union (F2 fix, `tm-mark-fixed-findings-rv2`).** `state.disregarded`/`state.fixed` are reset to `new Set()` before each reseed from the DOM. The old add-only seed was unsound *specifically because* the canvas is implemented server-side as a single morph-in-place file whose JS state survives across runs and whose `record_id`s are positional: a finding fixed/dimmed in run N stayed in the client set and was re-toggled onto the same-position id in run N+1, re-adding a green ✓/dim that the (correct, `run_id`-gated) server render had dropped — silently defeating the server-side "new `run_id` ⇒ clean canvas" reset. The lesson generalises to any future server-derived set on this canvas: reconcile from the current DOM, never accumulate. Regression lock: `test_review_canvas_template.py::test_restore_state_rebuilds_server_sets_fresh`.
