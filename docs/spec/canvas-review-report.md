...
- **`disregard` is the one stateful action — persisted in `{run_dir}/run-state.json`.** After confirmation the orchestrator re-runs `validate-action … --action focused-review.disregard --apply-disregard --run-dir {run_dir}`; `--apply-disregard` is the **only** side effect, is **bound to the `focused-review.disregard` verb** (pairing it with any other or absent action is rejected with exit 1 and writes **no** state — the gate is both an early fail-fast check and a guard co-located with the side effect), and fires **only after a successful validation** (a forged/unknown `run_id` or id can never write state), merging the ids into `run-state.json` monotonically (add-only, first-seen order). `render-review` reads it back (`load_run_state`, gated on the run's `run_id`) and bakes the `dimmed` class onto those `.finding` blocks, so the dim **persists across every re-render** — not just the in-session JS morph. The state file is **fail-open**: missing/unreadable/non-dict/JSON-error/mismatched-or-missing `run_id` all yield an empty disregarded set, so it never blocks the always-written canvas and never dims another run's findings.
