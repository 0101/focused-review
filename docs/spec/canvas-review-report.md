...
- `ids` are **stable** ids — finding `record_id`s (`r#`) and/or rule-quality-note ids (`RQ#`), never positional display numbers. `button` is the bare verb pressed (`fix`/`disregard`/`document`); `text` is the free-text instructions box.
...
- **Stable ids**: `record_id` required, non-empty, **unique**. `assessment_id` is nullable but, when present, must be **unique** (it locates the detail sidecar). `display_number` is required and **contiguous `1..N` per visible bucket** — `confirmed` / `needs-decision` / `pre-existing` each carry their own independent 1-based sequence, so a number is unique within its bucket but repeats across buckets — and is **null for the `hidden` bucket** (every Invalid finding plus any pre-existing Questionable, which are recorded in `records.json` but never rendered).
...
- **Records render/validation integrity (`focused-review-19p`, Phase 2/3 follow-up).** Two integrity fixes close gaps in the numbered-report contract: (1) `_md_finding_block` now `_flatten`s the `title` before it lands in the `### n. [sev] title` heading, so a raw CR/LF (or block markup) in a title can't inject a forged `### …` heading into `review.md` (which the post-mortem mode parses) — the table cells already collapsed newlines; the heading didn't. (2) `validate_records` now asserts the collected Confirmed/Questionable `display_number`s form a **contiguous** `1..N` run (not just unique ≥ 1), emitting an envelope-level error on a gap like `{1, 3}` — a reporter retry can renumber one finding and silently leave a gap that renders skipped numbers across all three surfaces. Locked by `test_render_review.py` (CRLF-title heading injection) and `test_records_schema.py` (`test_noncontiguous_display_numbers_rejected`).
