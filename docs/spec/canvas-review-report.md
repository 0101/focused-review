...
Phase 3 implements steps 1 and 3 above; the `nh3` sidecar sanitization (step 2) is deferred to Phase 4 (`focused-review-icp`) and the `capabilities`/`validate-action` surface (step 4) to later phases. Until step 2 lands, `render-review` renders the **text-field fallback only** (the fail-closed "plain escaped text" degradation baseline); `_canvas_finding_block` marks the sidecar extension point.
